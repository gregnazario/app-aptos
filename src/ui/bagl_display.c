...
static void ui_action_allow_blind_signing(const ux_flow_step_t* const* steps) {
    ...
    // Passed UX_FLOW steps are expected to contain a blind signing warning on
    // the first step. Skip it for better UX here.
    ux_flow_init(0, steps, steps[1]);
}

// Step with icon and text
UX_STEP_NOCB(ux_display_blind_sign_banner_step, pnn,
             {
                 &C_icon_warning,
...
UX_STEP_CB(ux_display_approve_step, pb, (*g_validate_callback)(true),
           {
               &C_icon_validate_14,
               "Approve",
           });
// Step with reject button
UX_STEP_CB(ux_display_reject_step, pb, (*g_validate_callback)(false),
           {
               &C_icon_crossmark,
               "Reject",
           });
// Step with the button to change settings
UX_STEP_CB(ux_display_allow_blind_sign_step, pnn,
...
UX_FLOW(ux_display_blind_sign_banner_flow, &ux_display_blind_sign_banner_step,
        &ux_display_allow_blind_sign_step, &ux_display_reject_step);
...
// Step with icon and text
UX_STEP_NOCB(ux_display_confirm_addr_step, pn,
             {&C_icon_eye, "Confirm Address"});
...
UX_STEP_NOCB(ux_display_address_step, bnnn_paging,
             {
                 .title = "Address",
                 .text = g_address,
             });
...
UX_FLOW(ux_display_pubkey_flow, &ux_display_confirm_addr_step,
        &ux_display_path_step, &ux_display_address_step,
...
// Step with icon and text
UX_STEP_NOCB(ux_display_blind_warn_step, pnn,
             {
                 &C_icon_warning,
                 "Blind",
                 "Signing",
...
// Step with icon and text
UX_STEP_NOCB(ux_display_review_step, pnn,
             {
                 &C_icon_eye,
                 "Review",
                 "Transaction",
...
// Step with icon and text
UX_STEP_NOCB(ux_display_review_msg_step, pnn,
             {
                 &C_icon_eye,
                 "Review",
                 "Message",
...
UX_STEP_NOCB(ux_display_msg_step, bnnn_paging,
             {
                 .title = "Message",
                 .text = (const char*)G_context.tx_info.raw_tx,
             });
// Step with title/text for message in short form
UX_STEP_NOCB(ux_display_short_msg_step, bnnn_paging,
             {
                 .title = "Message",
                 .text = g_struct,
             });
// Step with title/text for message in raw form
UX_STEP_NOCB(ux_display_raw_msg_step, bnnn_paging,
             {
                 .title = "Raw message",
                 .text = g_struct,
             });
// Step with title/text for transaction type
UX_STEP_NOCB(ux_display_tx_type_step, bnnn_paging,
             {
                 .title = "Transaction Type",
                 .text = g_tx_type,
             });
// Step with title/text for function
UX_STEP_NOCB(ux_display_function_step, bnnn_paging,
             {
                 .title = "Function",
                 .text = g_function,
             });
// Step with title/text for coin type
UX_STEP_NOCB(ux_display_coin_type_step, bnnn_paging,
             {
                 .title = "Coin Type",
                 .text = g_struct,
             });
// Step with title/text for amount
UX_STEP_NOCB(ux_display_amount_step, bnnn_paging,
             {
                 .title = "Amount",
                 .text = g_amount,
             });
// Step with title/text for receiver
UX_STEP_NOCB(ux_display_receiver_step, bnnn_paging,
             {
                 .title = "To",
                 .text = g_address,
             });
// Step with title/text for receiver
UX_STEP_NOCB(ux_display_pool_step, bnnn_paging,
             {
                 .title = "Pool",
                 .text = g_address,
             });
// Step with title/text for gas fee
UX_STEP_NOCB(ux_display_gas_fee_step, bnnn_paging,
             {
                 .title = "Gas Fee",
                 .text = g_gas_fee,
             });
...
UX_FLOW(ux_display_blind_tx_default_flow, &ux_display_blind_warn_step,
        &ux_display_review_step, &ux_display_tx_type_step,
        &ux_display_gas_fee_step, &ux_display_approve_step,
...
#define SEQUENCE_MESSAGE                               \
    &ux_display_review_msg_step, &ux_display_msg_step, \
        &ux_display_approve_step, &ux_display_reject_step
UX_FLOW(ux_display_message_flow, SEQUENCE_MESSAGE);
// preceding screen : warning icon + "Blind Signing"
UX_FLOW(ux_display_blind_message_flow, &ux_display_blind_warn_step,
        SEQUENCE_MESSAGE);
...
#define SEQUENCE_SHORT_MESSAGE                               \
    &ux_display_review_msg_step, &ux_display_short_msg_step, \
        &ux_display_approve_step, &ux_display_reject_step
UX_FLOW(ux_display_short_message_flow, SEQUENCE_SHORT_MESSAGE);
// preceding screen : warning icon + "Blind Signing"
UX_FLOW(ux_display_blind_short_message_flow, &ux_display_blind_warn_step,
        SEQUENCE_SHORT_MESSAGE);
...
#define SEQUENCE_RAW_MESSAGE                               \
    &ux_display_review_msg_step, &ux_display_raw_msg_step, \
        &ux_display_approve_step, &ux_display_reject_step
...
UX_FLOW(ux_display_blind_tx_entry_function_flow, &ux_display_blind_warn_step,
        &ux_display_review_step, &ux_display_tx_type_step,
        &ux_display_function_step, &ux_display_gas_fee_step,
        &ux_display_approve_step, &ux_display_reject_step);
...
UX_FLOW(ux_display_tx_aptos_account_transfer_flow, &ux_display_review_step,
        &ux_display_tx_type_step, &ux_display_function_step,
        &ux_display_receiver_step, &ux_display_amount_step,
        &ux_display_gas_fee_step, &ux_display_approve_step,
        &ux_display_reject_step);
...
// #7 screen : approve button
// #8 screen : reject button
UX_FLOW(ux_display_tx_listed_coin_transfer_flow, &ux_display_review_step,
        &ux_display_tx_type_step, &ux_display_function_step,
        &ux_display_amount_step, &ux_display_receiver_step,
        &ux_display_gas_fee_step, &ux_display_approve_step,
        &ux_display_reject_step);
...
UX_FLOW(ux_display_tx_unlisted_coin_transfer_flow, &ux_display_review_step,
        &ux_display_tx_type_step, &ux_display_function_step,
        &ux_display_coin_type_step, &ux_display_amount_step,
        &ux_display_receiver_step, &ux_display_gas_fee_step,
        &ux_display_approve_step, &ux_display_reject_step);
...
// #7 screen : approve button
// #8 screen : reject button
UX_FLOW(ux_display_tx_delegation_flow, &ux_display_review_step,
        &ux_display_tx_type_step, &ux_display_function_step,
        &ux_display_amount_step, &ux_display_pool_step,
        &ux_display_gas_fee_step, &ux_display_approve_step,
        &ux_display_reject_step);

// Generic arg display steps
UX_STEP_NOCB(ux_display_arg1_step, bnnn_paging,
             {.title = g_arg_labels[0], .text = g_arg_values[0]});
UX_STEP_NOCB(ux_display_arg2_step, bnnn_paging,
             {.title = g_arg_labels[1], .text = g_arg_values[1]});
UX_STEP_NOCB(ux_display_arg3_step, bnnn_paging,
             {.title = g_arg_labels[2], .text = g_arg_values[2]});
UX_STEP_NOCB(ux_display_arg4_step, bnnn_paging,
             {.title = g_arg_labels[3], .text = g_arg_values[3]});
UX_STEP_NOCB(ux_display_arg5_step, bnnn_paging,
             {.title = g_arg_labels[4], .text = g_arg_values[4]});
UX_STEP_NOCB(ux_display_arg6_step, bnnn_paging,
             {.title = g_arg_labels[5], .text = g_arg_values[5]});
UX_STEP_NOCB(ux_display_extra_info_step, bnnn_paging,
             {.title = "Note", .text = g_extra_info});
UX_STEP_NOCB(ux_display_multisig_addr_step, bnnn_paging,
             {.title = "Multisig", .text = g_multisig_addr});

// Generic entry function flows with 0-6 args
UX_FLOW(ux_display_generic_0_args_flow, &ux_display_review_step,
        &ux_display_tx_type_step, &ux_display_function_step,
        &ux_display_gas_fee_step, &ux_display_approve_step,
        &ux_display_reject_step);
UX_FLOW(ux_display_generic_1_args_flow, &ux_display_review_step,
        &ux_display_tx_type_step, &ux_display_function_step,
        &ux_display_arg1_step, &ux_display_gas_fee_step,
        &ux_display_approve_step, &ux_display_reject_step);
UX_FLOW(ux_display_generic_2_args_flow, &ux_display_review_step,
        &ux_display_tx_type_step, &ux_display_function_step,
        &ux_display_arg1_step, &ux_display_arg2_step, &ux_display_gas_fee_step,
        &ux_display_approve_step, &ux_display_reject_step);
UX_FLOW(ux_display_generic_3_args_flow, &ux_display_review_step,
        &ux_display_tx_type_step, &ux_display_function_step,
        &ux_display_arg1_step, &ux_display_arg2_step, &ux_display_arg3_step,
        &ux_display_gas_fee_step, &ux_display_approve_step,
        &ux_display_reject_step);
UX_FLOW(ux_display_generic_4_args_flow, &ux_display_review_step,
        &ux_display_tx_type_step, &ux_display_function_step,
        &ux_display_arg1_step, &ux_display_arg2_step, &ux_display_arg3_step,
        &ux_display_arg4_step, &ux_display_gas_fee_step,
        &ux_display_approve_step, &ux_display_reject_step);
UX_FLOW(ux_display_generic_5_args_flow, &ux_display_review_step,
        &ux_display_tx_type_step, &ux_display_function_step,
        &ux_display_arg1_step, &ux_display_arg2_step, &ux_display_arg3_step,
        &ux_display_arg4_step, &ux_display_arg5_step, &ux_display_gas_fee_step,
        &ux_display_approve_step, &ux_display_reject_step);
UX_FLOW(ux_display_generic_6_args_flow, &ux_display_review_step,
        &ux_display_tx_type_step, &ux_display_function_step,
        &ux_display_arg1_step, &ux_display_arg2_step, &ux_display_arg3_step,
        &ux_display_arg4_step, &ux_display_arg5_step, &ux_display_arg6_step,
        &ux_display_gas_fee_step, &ux_display_approve_step,
        &ux_display_reject_step);

// Script payload flows (no function name, just tx type + args)
UX_FLOW(ux_display_script_0_args_flow, &ux_display_review_step,
        &ux_display_tx_type_step, &ux_display_gas_fee_step,
        &ux_display_approve_step, &ux_display_reject_step);
UX_FLOW(ux_display_script_1_args_flow, &ux_display_review_step,
        &ux_display_tx_type_step, &ux_display_arg1_step,
        &ux_display_gas_fee_step, &ux_display_approve_step,
        &ux_display_reject_step);
UX_FLOW(ux_display_script_2_args_flow, &ux_display_review_step,
        &ux_display_tx_type_step, &ux_display_arg1_step, &ux_display_arg2_step,
        &ux_display_gas_fee_step, &ux_display_approve_step,
        &ux_display_reject_step);
UX_FLOW(ux_display_script_3_args_flow, &ux_display_review_step,
        &ux_display_tx_type_step, &ux_display_arg1_step, &ux_display_arg2_step,
        &ux_display_arg3_step, &ux_display_gas_fee_step,
        &ux_display_approve_step, &ux_display_reject_step);
UX_FLOW(ux_display_script_4_args_flow, &ux_display_review_step,
        &ux_display_tx_type_step, &ux_display_arg1_step, &ux_display_arg2_step,
        &ux_display_arg3_step, &ux_display_arg4_step, &ux_display_gas_fee_step,
        &ux_display_approve_step, &ux_display_reject_step);
UX_FLOW(ux_display_script_5_args_flow, &ux_display_review_step,
        &ux_display_tx_type_step, &ux_display_arg1_step, &ux_display_arg2_step,
        &ux_display_arg3_step, &ux_display_arg4_step, &ux_display_arg5_step,
        &ux_display_gas_fee_step, &ux_display_approve_step,
        &ux_display_reject_step);
UX_FLOW(ux_display_script_6_args_flow, &ux_display_review_step,
        &ux_display_tx_type_step, &ux_display_arg1_step, &ux_display_arg2_step,
        &ux_display_arg3_step, &ux_display_arg4_step, &ux_display_arg5_step,
        &ux_display_arg6_step, &ux_display_gas_fee_step,
        &ux_display_approve_step, &ux_display_reject_step);
...
int ui_display_message() {
    if (N_storage.settings.show_full_message) {
        ...
        snprintf(g_struct, sizeof(g_struct),
                 short_enough ? "%.*s" : "%.*s" DOTS,
                 short_enough ? G_context.tx_info.raw_tx_len
                              : sizeof(g_struct) - sizeof(DOTS),
...
UX_FLOW(ux_display_multisig_execute_flow, &ux_display_review_step,
        &ux_display_tx_type_step, &ux_display_multisig_addr_step,
        &ux_display_extra_info_step, &ux_display_gas_fee_step,
        &ux_display_approve_step, &ux_display_reject_step);
...
UX_FLOW(ux_display_multisig_hash_flow, &ux_display_review_step,
        &ux_display_tx_type_step, &ux_display_multisig_addr_step,
        &ux_display_function_step, &ux_display_gas_fee_step,
        &ux_display_approve_step, &ux_display_reject_step);
...
UX_STEP_NOCB(ux_display_tx_seq_step, bnnn_paging,
             {.title = "Transaction", .text = g_amount});

// Multisig vote flow: tx type + multisig addr + sequence + gas
UX_FLOW(ux_display_multisig_vote_flow, &ux_display_review_step,
        &ux_display_tx_type_step, &ux_display_multisig_addr_step,
        &ux_display_tx_seq_step, &ux_display_gas_fee_step,
        &ux_display_approve_step, &ux_display_reject_step);
